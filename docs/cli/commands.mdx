...
gaia update        # Pull, rebuild, migrate, restart (self-host mode)
...
Pull the latest code, rebuild images, run pending database migrations, and restart — the full upgrade flow in one command.

```bash
gaia update
```

Runs, in order: `git pull --ff-only` (refuses rather than merging/rebasing if your branch has diverged), `docker compose pull` + `build`, `docker compose up run-migrations` (aborts here on a failed migration, before anything restarts), then `docker compose up -d` to restart the full stack. Prints a summary: old → new git SHA, migrations applied/skipped, services running.

Self-host mode only — see [Upgrading](/self-hosting/upgrading) for the full flow, the manual equivalent, and how to read the migration runner's output.
...
See [Upgrading](/self-hosting/upgrading) for the full flow (migrations, backups, what can break) and the manual equivalent if you're not using the CLI.
