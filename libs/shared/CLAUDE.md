...
- **Infisical is optional in every environment** — zero Infisical env vars logs one line and proceeds on local env, including production (self-host runs on plain `.env`). A *partially* set machine-identity trio still raises `InfisicalConfigError` in production and warns in dev. Host/slug are overridable via `INFISICAL_HOST` / `INFISICAL_ENV`.
